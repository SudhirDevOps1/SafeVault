...
import {
  Shield, Key, Star, Wand2, Settings, Lock, Plus, Search, Keyboard, Download, Mail, CreditCard, ShieldAlert, Share2
} from 'lucide-react';
...
export default function Sidebar({ onAddCredential }: SidebarProps) {
  ...
  const navItems: { view: SidebarView; icon: React.ReactNode; label: string; count?: number; ariaLabel: string }[] = [
    ...
    { view: 'cards', icon: <CreditCard className="w-4 h-4" aria-hidden="true" />, label: 'Cards', count: credentials.filter(c => c.category === 'Payment Card').length, ariaLabel: 'Show payment cards' },
    { view: 'share', icon: <Share2 className="w-4 h-4" aria-hidden="true" />, label: 'Safe Share', ariaLabel: 'Open secure sharing panel' },
    { view: 'health', icon: <ShieldAlert className="w-4 h-4" aria-hidden="true" />, label: 'Health', ariaLabel: 'Open password health dashboard' },
  ...
  return (
    <div className="w-64 h-full bg-gray-950/50 border-r border-white/5 flex flex-col shrink-0" role="navigation" aria-label="Main navigation">
      ...
      {!(typeof window !== 'undefined' && 'electron' in window) && (() => {
        ...
        if (isMobile) {
          detectedOS = 'Android';
          directUrl = `https://github.com/SudhirDevOps1/SafeVault/releases/download/v${APP_VERSION}/SafeVault-v${APP_VERSION}.apk`;
          ...
          <div className="mx-3 my-2 p-3 bg-white/5 border border-white/5 rounded-xl">
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider">
                {isMobile ? 'Download Mobile App' : 'Download Desktop'}
              </span>
            </div>
            <p className="text-[11px] text-gray-500 mb-2.5 leading-relaxed">
              {isMobile ? 'Install SafeVault directly on Android' : 'Run securely offline on your device'}
            </p>
            ...
            >
              {isMobile ? 'Download Android APK' : `Download for ${detectedOS}`}
            </a>

            {/* Alternative Links */}
            <div className="flex items-center justify-between gap-1 text-[10px] text-gray-500">
              <span>{isMobile ? 'Alternative Downloads:' : 'Other platforms:'}</span>
              <a
