@echo off
:: ============================================================================
:: SafeVault - Automated Setup & Launch Tool (Advanced Self-Healing Edition)
:: ============================================================================
:: Fixes implemented:
::   1. UTF-8 Code Page 65001 enabled (fixes garbled "ƒöÆ" Unicode emoji bug)
::   2. ANSI 24-bit/16-color Escape sequences enabled for Windows 10/11 CMD
::   3. Double quotes around title (fixes "& Launch" command-split crash bug)
::   4. Automatic "npm link --force" (fixes EEXIST file already exists bug)
::   5. Auto-repair & cache clean fallback sequence for stuck dependencies
:: ============================================================================

:: [FIX #1] Set active code page to UTF-8 (65001) so unicode & emoji render cleanly
chcp 65001 >nul 2>&1

:: [FIX #2] Enable ANSI Color sequences in Command Prompt
...
echo %C_CYAN%   ╚══════╝╚═╝  ╚═╝╚═╝     ╚══════╝  ╚═══╝  ╚═╝  ╚═╝ ╚═════╝ ╚══════╝   ╚═╝           %C_RESET%
echo %C_CYAN%========================================================================================%C_RESET%
echo %C_GREEN%   🔒 SAFEVAULT : ALL-IN-ONE SECURE VAULT PLATFORM LAUNCHER & AUTO-REPAIR STUDIO        %C_RESET%
echo %C_CYAN%========================================================================================%C_RESET%
echo.
...
:: ============================================================================
echo %C_CYAN%[1/3] Checking Node.js & npm Environment...%C_RESET%
node -v >nul 2>&1
if %errorlevel% neq 0 (
    echo %C_RED%[ERROR] Node.js is not installed or not added to your system PATH!%C_RESET%
    echo %C_YELLOW%        Please install Node.js (v18+ LTS) from https://nodejs.org/%C_RESET%
    echo.
    pause
    exit /b 1
)
...
:: ============================================================================
:: [2/3] INSTALLING & AUTO-HEALING DEPENDENCIES
:: ============================================================================
echo %C_CYAN%[2/3] Checking & Installing project dependencies...%C_RESET%
...
if %errorlevel% neq 0 (
    echo %C_YELLOW%[WARNING] Standard npm install encountered warnings/errors. Running Auto-Repair...%C_RESET%
    echo   %C_MAGENTA%---> Cleaning npm cache & retrying with --legacy-peer-deps...%C_RESET%
...
:menu
echo %C_CYAN%========================================================================================%C_RESET%
echo %C_WHITE%%C_BOLD%               SAFEVAULT MULTI-PLATFORM LAUNCH & DIAGNOSTICS SUITE                %C_RESET%
echo %C_CYAN%========================================================================================%C_RESET%
...
echo %C_GREEN% [5]%C_RESET% Build & Start Web Production Server (%C_YELLOW%npm run build && npm start%C_RESET%)
echo %C_GREEN% [6]%C_RESET% Run CLI Interactive Help (%C_MAGENTA%safevault --help%C_RESET%)
echo %C_GREEN% [7]%C_RESET% Deep Auto-Repair & Clean Reset (%C_RED%Purge node_modules + Force Relink%C_RESET%)
echo %C_GREEN% [8]%C_RESET% System Health Check & Port Inspector (%C_BLUE%Verify Ports 3000, 5173, 8080%C_RESET%)
...
if "%choice%"=="1" (
    ...
    echo.
    echo %C_MAGENTA%Running SafeVault CLI Help & Command List...%C_RESET%
    ...
    echo.
    echo %C_RED%[AUTO-REPAIR] Executing Deep Reset & Dependency Cleanup...%C_RESET%
    ...
    echo.
    echo %C_BLUE%=== SafeVault Environment & Port Diagnostics ===%C_RESET%
    ...
    echo           The line 'title SafeVault - Automated Setup ^& Launch Tool' without quotes
    echo           causes cmd.exe to split at '^&' and try to execute 'Launch' as a command!
    echo    %C_GREEN%Fix  :%C_RESET% Wrapped the title in double quotes: "SafeVault - Automated Setup ^& Launch Tool"
    echo.
    ...
    echo.
    echo %C_BOLD%3. BUG: "ƒöÆ" Garbled Unicode Emojis in Terminal%C_RESET%
    echo    %C_RED%Cause:%C_RESET% Default Windows Command Prompt uses Code Page 437 (DOS ASCII), corrupting emojis.
    echo    %C_GREEN%Fix  :%C_RESET% Added 'chcp 65001 >nul 2>^&1' at line 1 for native UTF-8 and ANSI colors.
    echo.
    echo %C_YELLOW%============================================================================%C_RESET%
    echo.
    pause
